...
public class SagitCapture {
	...
	public SagitCapture() { // Default constructor
		devices = JpcapCaptor.getDeviceList(); // Feeds devices all network
												// interfaces on the machine
	}

	public SagitCapture(int deviceIndex) {
		this();
		this.openDevice(deviceIndex);
	}
	...
	/**
	 * This opens a device from the list of devices with a specific index
	 * 
	 * @param index
	 * @returns void
	 */
	public void openDevice(int index) { // Opens the device takes in index you
		...
		try { // beginning of a try catch block
			captor = JpcapCaptor.openDevice(devices[index], 1028, false, 20); // Opens
																				// the
																				// selected
																				// device
...
/*
 * 
 * Method to hold the received packets in an arrayList of packets
 * 
 */ 
	...
	public void acquirePacketsTimed(final List<Packet> buffer, final int count,
			long milliseconds) {
	...
	/*
	 * public static void main(String[] args){ //Main Method SagitCapture cap =
	 * new SagitCapture(); //Creates a new captor object cap.openDevice(0);
	 * //Opens the device 1 List<Packet> buffer =
	 * Collections.synchronizedList(new ArrayList<Packet>()); //Creates a new
	 * list for the packets cap.acquirePackets(buffer, -1); //making it crash
	 * find the problem }
	 */
}
