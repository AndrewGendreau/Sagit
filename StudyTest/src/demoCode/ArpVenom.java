...
public class ArpVenom {
	...
	public ArpVenom() { //Default Constructor
		devices = JpcapCaptor.getDeviceList(); //Fills devices with the devices on the machine
	...
	public EthernetPacket etherGen(int deviceIndex, ARPPacket p) { // Helper
		...
		ether.frametype = EthernetPacket.ETHERTYPE_ARP; //Makes the frame type ARP
		ether.src_mac = devices[deviceIndex].mac_address; //Sets the source mac to the opened device
	...
	public byte[] getIP(int deviceIndex) { //Grabs the IP address of the chosen device
		...
		InetAddress temp; //Variable to hold the Inet address representation of the Ip address
		byte[] address; //Variable to hold the byte representation
			temp = Inet[0].address; 
			//Sets Temp equal to element 0 in the address array which is the IP address
			address = temp.getAddress(); 
			//Calls getAddress to grab the byte array representation and places it in address
